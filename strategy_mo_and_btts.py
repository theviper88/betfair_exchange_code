...
def mo_and_btts(sessionToken, appKey, \
                correctscoreBook, moandbttsBook, \
                correctscoreID, moandbttsID, strategy_name, \
                trading_stake, max_runner_liability, max_market_liability, margin_multiplier_1, margin_multiplier_2, \
                old_moandbtts_tolay_prices, lay_bet_ids, old_moandbtts_toback_prices, back_bet_ids, \
                selectionIds, selectionIds_liability, pnl_positions, ev_trend):
                
        # get market details
        correctscoreDeets = lib_api_commands.getMarketDeets(appKey, sessionToken, correctscoreID)
        moandbttsDeets = lib_api_commands.getMarketDeets(appKey, sessionToken, moandbttsID)
        #correctscoreBook = lib_api_commands.getSelections(appKey, sessionToken, correctscoreID)
        #moandbttsBook = lib_api_commands.getSelections(appKey, sessionToken, moandbttsID)
        marketId = moandbttsBook[0]['marketId']
